...
export default function AddMealModal({ visible, onClose, onAddMeal, isDarkMode }) {
  ...
  const [descriptionAdditions, setDescriptionAdditions] = useState({ additions: [], extraCal: 0 });
  const [parsedWeight, setParsedWeight] = useState(null);

  // Live description parser
  useEffect(() => {
    if (!description.trim()) {
  ...
  const handlePickImage = async (useCamera = false) => {
    try {
      ...
      if (!result.canceled && result.assets?.[0]?.uri) {
        ...
        if (visionRes.isValidFood) {
          setSelectedRecipe({
            name: visionRes.foodName,
            cal: visionRes.cal,
            protein: visionRes.protein,
            carbs: visionRes.carbs,
            fat: visionRes.fat,
      ...
      <View style={styles.overlay}>
        <View style={[styles.sheetContainer, { backgroundColor: bgColor }]}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            {/* Selected Recipe & Portion Multipliers */}
            {selectedRecipe && (
