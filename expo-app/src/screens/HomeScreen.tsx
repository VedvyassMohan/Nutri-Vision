...
import { User, Macros, WaterData, StreakData } from '../types';
...
function CalorieRing({ consumed, goal, isDarkMode }: { consumed: number; goal: number; isDarkMode: boolean }) {
...
export default function HomeScreen({ user, isDarkMode }: HomeScreenProps) {
  const [streakData, setStreakData] = useState<StreakData>({ currentStreak: 1, weekDays: [] });
  const [macros, setMacros] = useState<Macros>({ calories: 0, protein: 0, carbs: 0, fat: 0 });
  const [waterData, setWaterData] = useState<WaterData>({ glasses: 0, goal: 8, targetLiters: 2.0, timerEnabled: true, intervalMinutes: 25 });
